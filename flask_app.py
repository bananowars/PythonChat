...
def change_password(password_hash):
    select(r for r in Users if r.id == get_userid()).get().password = password_hash
    commit()
...
def api_messages_oldmessagelist():
        ...
        messages = [[escape(x.name), escape(x.message), datetime.strftime(x.time, "%d.%m.%Y %H:%M:%S")] for x in msg]
        if len(msg) == 0:
...
def api_messages_newmessage():
        msg = get_message()
        message = [[escape(x.name), escape(x.message), datetime.strftime(x.time, "%d.%m.%Y %H:%M:%S")] for x in msg]
        if len(msg) == 0:
            return jsonify(error = "False")
